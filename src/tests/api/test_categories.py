...
async def test_get_categories(client: AsyncClient, headers, status_code, res_body):
...
async def test_add_category(client: AsyncClient, headers, data, status_code, res_body):
    res = await client.post(
        f"{config.API_V1_STR}/categories", headers=headers, json=data
    )
    assert res.status_code == status_code
    assert res.json() == res_body
...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    "headers, category_id, status_code, res_body",
    [
...
async def test_delete_category_failure(
    client: AsyncClient, headers, category_id, status_code, res_body
):
    res = await client.delete(
        f"{config.API_V1_STR}/categories/{category_id}", headers=headers
    )
    assert res.status_code == status_code
    assert res.json() == res_body
...
async def test_delete_category_success(
    ...
    res = await client.delete(
        f"{config.API_V1_STR}/categories/3", headers=user_token_headers
    )
