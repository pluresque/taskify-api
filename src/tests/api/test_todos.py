...
API_TODOS_PREFIX: Final[str] = f"{config.API_V1_STR}/src"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lazy_fixture("user_token_headers"), 200, get_tests_data()["users"][0]["src"]),
    ],
    ids=["unauthorized access", "authorized access"],
)
async def test_get_todos(client: AsyncClient, headers, status_code, res_body):
...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    "headers, data, status_code, res_body",
    [
        (None, {}, 401, {"detail": "Unauthorized"}),
        (
...
async def test_add_todo(client: AsyncClient, headers, data, status_code, res_body):
...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    "headers, todo_id, data, status_code, res_body",
    [
...
async def test_update_todo(
    client: AsyncClient, headers, todo_id, data, status_code, res_body
):
...
@pytest.mark.asyncio
@pytest.mark.parametrize(
    "headers, todo_id, status_code, res_body",
    [
        (None, 1, 401, {"detail": "Unauthorized"}),
...
async def test_delete_todo_failure(
    client: AsyncClient, headers, todo_id, status_code, res_body
):
