...
from app.schemas import TodoCreate, TodoInDB, TodoRead, TodoUpdate, TodoUpdateInDB
...
@router.put(
    ...
    responses={
        ...
        status.HTTP_403_FORBIDDEN: get_open_api_response(
            {
                "Trying to update another users todo": "a user can not update a todo that was not created by him"
            }
        ),
...
@router.delete(
    ...
    responses={
        ...
        status.HTTP_404_NOT_FOUND: get_open_api_response(
            {"Trying to update non existing todo": "todo does not exist"}
        ),
    },
)
@exception_handler
